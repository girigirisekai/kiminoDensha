...
public class StationNameToPath2 {
			// 역 이름과 이름으로 경로 만들기
		...
		if (conn.getResponseCode() >= 500) { // 에러団 처리
			...
			try {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					String shtStatnNm2 = person_E.getChild("shtStatnNm").getValue();
					String shtTravelMsg = person_E.getChild("shtTravelMsg").getValue();
					String shtStatnNm = shtStatnNm2.replaceAll(" ", "");
					System.out.println(shtStatnNm);
					//sList = (ArrayList<String>) Arrays.asList(shtStatnNm.split(" ,"));
					String [] array = shtStatnNm.split(",");
