		...
		<div id="gesipan" style="width: 80%;">
			...
			<td colspan="4" style="margin: auto; text-align: center; font-weight:bold;"><h1>${type}게시판</h1></td>
				...
				<tbody>
					...
					${list.title.replace("<", "&lt;") }
					</a> --%> <span class="titleNames" boardnum="${list.boardnum}"
								type="${type}"> ${list.title.replace("<", "&lt;") }
							</span> &nbsp;&nbsp; <a href="#"
					...
					<tr>
						<td colspan="5" class="info" style="text-align: center"><a
							...
							<a href="javascript:formSubmit('${navi.currentPage-1}')"></a> <c:forEach
								var="i" begin="${navi.startPageGroup}"
					...
					<tr>
						<form id="pagingForm" action="board" method="get">
							...
							<td colspan="5" style="text-align: right">제목 : <input
	...
	<div id="passwordCheckPopup"
		...
		<table>
			<form id="passwordCheckTable">
				<tr>
					<th><input type="hidden" name="boardnum" id="checkBoardnum">
						<input type="hidden" name="type" id="checkType"> <input
						type="password" name="secretpassword" id="checkPassword">
					<th>
				</tr>
				<tr>
					<td><input type="button" value="비밀번호 확인" id="checkButton">
					</td>
				</tr>
			</form>
		</table>
